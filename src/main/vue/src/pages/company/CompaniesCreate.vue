<template>
  <v-app id="companies-create-page">
        ...
        <v-flex xs12 sm12 md12>
          <v-card class="elevation-2">
            <v-toolbar color="white" class="elevation-1 container-center">
              <div>
                <v-toolbar-title>Criar Empresa</v-toolbar-title>
              </div>
            </v-toolbar>
...
<script>
  ...
  export default {
    name: 'companies-create-page',
    methods: {
      validate: function() {
        if(!this.company.name) {
          this.error = "Você precisa especificar um nome válido";
          return false;
        }
        return true;
      },
      ...
      save: function (event) {
        ...
        if(!this.snackbar){
          axios.post(CONFIG.URL_API + 'companies', companyData, { headers: headers })
            .then(response => {
              this.dialog = false;
